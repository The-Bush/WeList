...
def home():
    ...
    if request.method == "POST":
        ...
        if 'access' in request.form:
            ...
            rows = db.execute('SELECT id, name, password_hash FROM lists WHERE name = ?', (listname, ))
            rows = rows.fetchone()
            app.logger.debug("Found List", rows)
            connection.close()

            # Check if the list exists and if password matches
            if rows is None:
...
def createlist():
    ...
    if request.method == "POST":
        ...
        db.execute('INSERT INTO lists (name, password_hash, last_used) VALUES(?, ?, ?)', (listname, hashPassword, datetime.datetime.now()))   
        connection.commit()
...
def viewList():
    ...
    if request.method == "POST":
        ...
        connection, db = create_db_connection()
        rows = db.execute('SELECT name, qty, cost_each, id FROM items WHERE list_id = ?', (session['list_id'], ) )
        items = rows.fetchall()
        connection.close()
...
def add_item():
    if request.method == "POST":
        ...
        connection, db = create_db_connection()
        db.execute('INSERT INTO items (list_id, name, qty, cost_each) VALUES(?, ?, ?, ?)', (session['list_id'], itemName, qty, costEach))   
        connection.commit()
        connection.close()
